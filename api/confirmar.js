...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { data, error } = await supabaseAdmin
      .from('agendamentos')
      .select('id,status,cliente_nome,data_agendamento,hora_inicio,barbearias(nome)')
      .eq('id', id)
      .maybeSingle();
    if (error) throw error;
    if (!data) return json(res, 404, { erro: 'Agendamento não encontrado.' });
    return res.status(200).send(`<!doctype html><meta name="viewport" content="width=device-width,initial-scale=1"><body style="font-family:Arial;background:#090909;color:#fff;display:grid;place-items:center;min-height:100vh;text-align:center;padding:24px"><div><h2>Agendamento encontrado ✅</h2><p>${data.cliente_nome}, seu horário na ${data.barbearias?.nome || 'barbearia'} está como <b>${data.status}</b>.</p><p>${data.data_agendamento} às ${String(data.hora_inicio).slice(0,5)}</p><small>Para confirmar presença pelo WhatsApp, responda 1 na mensagem de lembrete.</small></div></body>`);
  } catch (err) {
