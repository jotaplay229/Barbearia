...
const STORE_META_PREFIX = 'BARBEROS_STORE_META:';

function parseStoreMeta(value) {
  const text = String(value || '');
  if (!text.startsWith(STORE_META_PREFIX)) return { descricao: text, horarios_custom: {}, financeiro: { lancamentos: [] } };
  try {
    const meta = JSON.parse(text.slice(STORE_META_PREFIX.length));
    return {
      descricao: meta.descricao || '',
      horarios_custom: meta.horarios_custom || {},
      financeiro: meta.financeiro || { lancamentos: [] }
    };
  } catch {
    return { descricao: '', horarios_custom: {}, financeiro: { lancamentos: [] } };
  }
}

export function storeMetaDescription({ descricao, horarios_custom, financeiro } = {}) {
  return STORE_META_PREFIX + JSON.stringify({
    descricao: String(descricao || '').trim(),
    horarios_custom: horarios_custom || {},
...
export function normalizeBarbearia(row = {}) {
  const meta = parseStoreMeta(row.descricao);
  return {
    ...row,
    descricao: meta.descricao,
    horarios_custom: row.horarios_custom || meta.horarios_custom || {},
    financeiro: row.financeiro || meta.financeiro || { lancamentos: [] },
