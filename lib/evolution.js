...
export async function evolutionRequest({ apiUrl, apiKey, path, method = 'GET', body }) {
  ...
  const resp = await fetch(`${base}${path}`, {
    ...
    if (err.name === 'AbortError') {
      throw new Error('Evolution demorou para responder. Confira se a URL da API está online e pública.');
    }
    throw err;
...
export async function setWebhook({ apiUrl, apiKey, instanceName, webhookUrl }) {
  ...
  try {
    ...
    if (msg.includes('404') || msg.includes('not found')) {
      return {
        aviso: 'Esta versão da Evolution não aceitou /webhook/set. O webhook enviado na criação da instância ainda será usado se a instância foi criada agora.',
        erro: err.message
      };
    }
    throw err;
