...
const SEND_RETRY_DELAYS_MS = [900, 1800, 3000];
...
function isTransientEvolutionError(err) {
...
async function retryTransient(fn) {
  ...
  for (let attempt = 0; attempt <= SEND_RETRY_DELAYS_MS.length; attempt++) {
    try {
      ...
      lastErr = err;
      if (!isTransientEvolutionError(err) || attempt === SEND_RETRY_DELAYS_MS.length) {
        throw err;
      }
      await wait(SEND_RETRY_DELAYS_MS[attempt]);
    }
  }
  throw lastErr;
}
...
export function legacyWebhookPayload(webhookUrl) {
  return {
    webhook: {
      enabled: true,
      url: webhookUrl,
...
function defaultInstanceSettings() {
  return {
    rejectCall: false,
    msgCall: '',
    groupsIgnore: true,
...
export async function createInstance({ apiUrl, apiKey, instanceName, webhookUrl, number }) {
  ...
  if (webhookUrl) body.webhook = webhookPayload(webhookUrl);
...
export async function setWebhook({ apiUrl, apiKey, instanceName, webhookUrl }) {
  ...
  try {
    return await evolutionRequest({
      ...
      body: legacyWebhookPayload(webhookUrl)
    });
  } catch (err) {
    const msg = String(err.message || '').toLowerCase();
    try {
      ...
      if (msg.includes('404') || msg.includes('not found') || fallbackMsg.includes('404') || fallbackMsg.includes('not found')) {
        return {
...
export async function findWebhook({ apiUrl, apiKey, instanceName }) {
  return evolutionRequest({
    apiUrl,
    apiKey,
    method: 'GET',
