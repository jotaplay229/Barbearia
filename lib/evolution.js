...
export async function evolutionRequest({ apiUrl, apiKey, path, method = 'GET', body }) {
  ...
  if (/^https?:\/\/(localhost|127\.0\.0\.1|\[::1\])/i.test(base)) {
    throw new Error('A Evolution esta como localhost. Publique a Evolution em uma URL publica e coloque essa URL na Vercel.');
  }
  ...
  const resp = await fetch(`${base}${path}`, {
    ...
    throw new Error(`Nao foi possivel conectar na Evolution em ${base}. Confirme se a URL publica e a API Key estao corretas.`);
